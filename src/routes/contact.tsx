      ...
      <div className="grid gap-8 lg:grid-cols-[1.2fr_1fr]">
        <form
          onSubmit={(e) => {
            ...
            void submitLead({ name, email, phone, form: "Contact page form", note: String(data.get("message") ?? "").slice(0, 200) });
            e.currentTarget.reset();
          }}
          className="surface-card space-y-4 p-7"
        >
          {[["Full name","text","name"],["Email address","email","email"],["Phone number","tel","phone"]].map(([label, type, fieldName]) => (
            <div key={label}>
              <label className="mb-1.5 block text-sm font-medium" htmlFor={label}>{label}</label>
              <input id={label} name={fieldName} required type={type} className="h-11 w-full rounded-xl border border-border bg-card px-4 text-sm outline-none focus:border-brand" placeholder={label} />
            </div>
          ))}
          <div>
            <label className="mb-1.5 block text-sm font-medium" htmlFor="msg">How can we help?</label>
            <textarea id="msg" name="message" rows={4} className="w-full rounded-xl border border-border bg-card px-4 py-3 text-sm outline-none focus:border-brand" placeholder="Tell us about your background and goal" />
          </div>
