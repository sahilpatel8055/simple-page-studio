 ...
 */
import { findCannibalisation, routeForQuery, siteIntentClaims, INTENT_OWNERSHIP } from "../src/lib/intentMap";
import { classifyKeyword, courseKeywordCluster, universityKeywordCluster } from "../src/lib/keywordClusters";
import { programmes, universities } from "../src/data";

const claims = siteIntentClaims();
const byKind = claims.reduce<Record<string, number>>((acc, c) => ({ ...acc, [c.kind]: (acc[c.kind] ?? 0) + 1 }), {});

console.log("Pages with a declared primary intent:", claims.length);
for (const [kind, n] of Object.entries(byKind)) {
  console.log(`  ${kind.padEnd(18)} ${String(n).padStart(4)}  owns: ${INTENT_OWNERSHIP[kind as keyof typeof INTENT_OWNERSHIP]?.owns ?? "-"}`);
}
...
console.log(`  universities mapped: ${universities.filter((u) => universityKeywordCluster(u.slug)).length}/${universities.length}`);
...
for (const q of samples) {
  ...
  console.log(`  ${q.padEnd(32)} ${c.intent.padEnd(24)} ${c.cluster.padEnd(14)} -> ${r ? r.path : "no owner"}`);
}
